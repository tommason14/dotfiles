...
map gh cd ~
# map gd cd /scratch/06233/tmason/dopamine
# map gt cd /scratch/06233/tmason/tests
# map gp cd /scratch/06233/tmason/polymers/polymerisation_rates
...
map sp shell cp ~/monash_automation/settings_files/gamess_spec.py .
map he shell cp ~/monash_automation/settings_files/gamess_hess.py .
map ds shell chem_assist -ds *py # find way to select which python script?
...
# MP2
map pm shell -w cat %s | grep 'E(MP2)' | sed '/NaN/d' | tr -s [:blank:] | cut -d ' ' -f 3 | gnuplot -e "set terminal dumb; plot '-' with lines notitle"
# FMO 
map pfo shell -w cat %s | grep 'E corr MP2(2)=' | tr -s [:blank:] | cut -d ' ' -f 10 | gnuplot -e "set terminal dumb; plot '-' with lines notitle"
# Gaussian
map pg shell -w cat %s | grep 'SCF Done' | tr -s [:blank:] | cut -d ' ' -f 6 | gnuplot -e "set terminal dumb; plot '-' with lines notitle"
